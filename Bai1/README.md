...
```bash
# Với file mặc định (là file alice30.txt)
mingw32-make run
 
# Với file tùy chỉnh (file vanban.txt)
mingw32-make run INPUT=vanban.txt OUTPUT=output2.txt
 
# Hoặc chạy trực tiếp
./indexer.exe <input_file> <output_file>
```
 
## Ví dụ
 
```bash
./indexer.exe alice30.txt output.txt
```
